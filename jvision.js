var jvision = new function() {
  this.Localizer = function(previewDiv, previewWidth, nBalls) {
    ...
    this.run = async function(finishedCallback) {
      ...
      const model = await tf.loadModel('./models/submovavgmodel_js/model.json');
      ...
      if (navigator.mediaDevices.getUserMedia) {
        ...
        .catch(function(e) {
          alert("Couldn't access webcam! Refresh and ensure you allow webcam access to run the demo.")
      ...
      function getBallAndHandCoordinates(prediction) {
        ...
        if (tf.max(rhand_pred.slice([0,0,0],[15,15,1])) > tf.max(lhand_pred.slice([0,0,0],[15,15,1]))) {
          ...
          reverse_hand_coordinates = getCoordinates(rhand_pred, 2, lhand_coordinates)
          hand_coordinates = []
        ...
        coordinates = coordinates.concat(getCoordinates(prediction.slice([0,0,0],[15,15,3]), nBalls=nBalls))
        return coordinates
      }

      function getCoordinates(prediction, nBalls, coordinates=[]) {
        prediction = tf.buffer([15,15,3], dtype='float32', prediction.dataSync())
        xIndex = 0
        yIndex = 0
        while (coordinates.length < nBalls*2) {
          ...
          for (i=0; i<coordinates.length; i+=2) {
            if (Math.sqrt((newX-coordinates[i])*(newX-coordinates[i]) + (newY-coordinates[i+1])*(newY-coordinates[i+1])) < Math.SQRT2 * 0.04) {
              valid = false
            }
          }
          if (valid) {
      ...
      function callback() {
        sourceCtx.drawImage(video, (video.videoWidth - video.videoHeight) / 2, 0, video.videoHeight, video.videoHeight, 0, 0, 64, 64);
        previewCtx.drawImage(video, (video.videoWidth - video.videoHeight) / 2, 0, video.videoHeight, video.videoHeight, 0, 0, previewWidth, previewWidth);
        coordinates = tf.tidy(() => {
          ...
          movavg.assign(tf.movingAverage(movavg, img, 0.850, 0, false))
    ...
    this.predict = function(coordinates) {
      ...
      if (coordinateHistory.length > length) {
        ...
        patternIndex = tf.tidy(() => {
          ...
          prediction = tf.squeeze(pattern_model.predict(tf.expandDims(pattern)))
  ...
  this.drawBallsAndHands = function(ctx, coordinates, nBalls) {
    ...
    ctx.beginPath()
    ctx.moveTo(coordinates[0]*previewWidth-12, coordinates[1]*previewWidth)
    ctx.lineTo(coordinates[0]*previewWidth+12, coordinates[1]*previewWidth)
    ctx.strokeStyle = "#00FF00"
    ctx.stroke()

    ctx.beginPath()
    ctx.moveTo(coordinates[2]*previewWidth-12, coordinates[3]*previewWidth)
    ctx.lineTo(coordinates[2]*previewWidth+12, coordinates[3]*previewWidth)
